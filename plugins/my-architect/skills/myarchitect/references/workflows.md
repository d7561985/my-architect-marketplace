...
1. **Вход-фильтр «две корзины»** (initiative-gate.md → Процесс). Работа ≤ пары недель И не меняет межсервисные контракты → ответить «это себестоимость команды, инициатива не оформляется» и **остановиться** — это успешный исход, не сбой. Если работу всё равно нужно трекать — указать на `/my-architect:feature` (Workflow Z).
...
4. **Спина → PROPOSE-then-CONFIRM.** 7 ответов по одному предложению каждый → показать автору → **WAIT**. Легитимные финалы здесь: «Опция 0 выигрывает — не делаем» и «не сейчас + триггер пересмотра: <условие>» — успешный результат работы. При «не сейчас» предложить (не навязывать) future-ноду с observable-триггером в description (rubric lane 2, no ask).
...
6. **Канон текста — один.** По умолчанию: файл эпика в репо автора (артефакт для CTO/BO; путь — конвенция репо, иначе спросить один раз), док на my_architect-ноде — та же структура с `**Source:** <путь файла>`, синхронизируется тем же ходом (ship = sync). Файл не нужен → канон сразу док на ноде. Никогда не два независимых канона.
...
8. **Финальный самопроход + verify.** Текст против initiative-gate.md → Правила текста: стоп-лист, тест переворачиваемости, модальность ЦЕЛЬ/СЕЙЧАС. Незакрытые `[факт: …]` перечислить в выдаче явно — это блокеры публикации. `validate_project` → чисто; echo созданных ID.
...
2. **Пять проверок** (категории слабостей):
   1. *Generic-фразы* — стоп-лист + тест переворачиваемости: отрицание фразы бессмысленно («не-опора для авторизации»?) → фраза пустая, нужна конкретика.
   ...
   3. *Статус-кво-тест* — на возражение «и так же работает — зачем трогать?» текст отвечает в том же разделе, а не полагается на устный комментарий.
   4. *«Если не делать»* — раздел присутствует и измерим: тренд на горизонте 6–12 месяцев + честный do-minimum, не «будет плохо».
