...
3. **Upfront-требования** (не «потом»). У фичи с тестируемой приёмкой — `add_requirement` сразу на feature-ноду:
   ...
   - ценность не заканчивается артефактом: актор + новое поведение + сигнал («сделаем X → Y сможет Z → увидим по W»);
   - модальность: целевые числа и состояния помечены как цель, не поданы как текущее;
   - description ноды проходит стоп-лист.
