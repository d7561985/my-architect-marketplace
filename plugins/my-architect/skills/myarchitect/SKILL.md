...
**Version:** 1.11 (2026-06-29). Bump: **Event Storming канвас** — новый тип диаграммы `event-storming` для доменно-событийного моделирования: `create_diagram({diagramType:'event-storming', dsl})` или ` ```event-storming `-блок в доке; построчный agent-native DSL (`group` рамки + `[event|command|policy|readmodel|actor|aggregate|external|hotspot]` + `connect`), создаётся и из иерархии. Гайд — `references/forming-nodes.md`. **Compatibility:** требует сборки app + `@my-architect/mcp`, где зарегистрирован тип `event-storming` (feature-006); до раската в my-architect.app `create_diagram` отвергает этот тип. (1.10 ниже.)
