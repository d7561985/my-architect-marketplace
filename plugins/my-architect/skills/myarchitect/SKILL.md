...
**Version:** 1.8 (2026-06-24). Bump: feature-altitude guidance (story-074) — `references/forming-nodes.md` gains an **Altitude** section (фича = building block: вертикальный наблюдаемый срез, не задача по слоям кода; merge-test + demo-test pre-flight перед `build_hierarchy`; title-grammar результата, не артефакта; «как»+file:line → acceptance/доку; cross-cutting≠фича; порядок=deps+релизы; granularity-budget smell), with a lean pointer from the core map. Catches *altitude* (vertical-slice vs task), complementing the RFC-013 *formulation* lint. (1.7 — progressive-disclosure refactor)
