...
## Setup — выбрать проект и сохранить связь с репозиторием

При первом обращении к проекту и в `/my-architect:init` прочитай [references/setup.md](references/setup.md).

1. Прочитай `CLAUDE.md` и `.architect/traceability.json`: используй согласованный явный `pid`/`projectId` или уже сделанный пользователем выбор. Конфликт ID требует уточнения до записи.
2. Если связи нет, получи `list_projects` и предложи выбрать существующий проект или создать новый. Даже единственный или первый результат не доказывает связь с репозиторием. Ошибка запроса не означает пустой список.
3. Создание выполняй по явному решению пользователя, с согласованными именем и `preset`, без sample content. Уже данное разрешение повторно не запрашивай.
4. Проверь выбранный/возвращённый ID через `get_project_context`, сохрани его точно в локальной привязке и продолжи `/my-architect:init`. Пользователь не обязан знать `pid` заранее; для MCP-операций с проектом он обязателен. Полная процедура и обработка незавершённой настройки — в setup reference.

Используй подтверждённую связь в следующих сессиях без повторного выбора. Отсутствие `.architect/` означает отсутствие локальной настройки, а не отсутствие проекта на сервере.
...
## Первый вызов с `pid` — после Setup

После разрешения ID прочитай контекст, если он ещё не прочитан в этом ходу при Setup:
...
- **Граф кода — перед любой фазой Discovery.** Есть `graphify-out/graph.json` (проверка — один `Bash`-вызов) → ориентация идёт запросом к графу, и только потом `grep`/`Read`/`Workflow`/fan-out. Порядок и его границы — `recursive-context` → [`references/code-graph.md`](../recursive-context/references/code-graph.md), раздел «Композиция». Плагин ставит SessionStart-хук, который сообщает о наличии и свежести графа сам; если сообщения не было — проверь руками, хук молчит и когда графа нет.
- **`design` (этот же плагин) вместо `superpowers:brainstorming`** — дизайн-сессии в my_architect-проектах ведёт skill `design` (форк brainstorming: explore через get_project_context + граф кода + recursive-context, спека доком на узле, терминал Workflow Z → writing-plans). Если всё же сработал оригинальный brainstorming — переопредели его шаги: (1) *Explore project context* → шаг 0 + Explore из design (get_project_context, code-graph, recursive-context); (2) *Write design doc* → канон = док на узле (`create_doc`, Workflow C), не `docs/superpowers/specs/` (локальный черновик — только промежуток, перенос на узел в том же ходу); (3) *Transition to implementation* → сперва Workflow Z (ноды + требования), план writing-plans = файл-канон + зеркальный док на ноде с `**Source:**` («канон один»). Длинная правка доки — `update_doc` find/replace/section, не перезапись целиком.
- **`superpowers:systematic-debugging`** — его Phase 4.5 (3+ неудачных фикса = архитектурная проблема) — вход в архитектора: STOP + Workflow I (структурное) или Workflow B (нода долга), а не четвёртый фикс.
- **`superpowers:finishing-a-development-branch`** — merge/PR = отгрузка: в том же ходу `complete_task`/`update_node` (ship = sync).

---

**Version:** 1.20.0 (2026-09-13, aligned with plugin version). **Setup:** явный выбор/создание проекта, проверенная локальная привязка и повторное использование без нового вопроса; порядок/размер списка не доказывает связь с репозиторием. **Traceability:** обязательный TDD + Trace; явный requirement `done` после доказанного исполнения; issue закрывается при завершении **всех** closing-элементов; синк и валидация в том же ходу. **Compatibility:** issue tools, `closes`, requirement `done` и проверки issue требуют совместимых backend + `@my-architect/mcp` ≥ 1.8.0; проверяй живую схему, не только номер версии. В `@my-architect/mcp` ≥ 1.8.2 также исправлена подсказка `list_projects` о выборе проекта.
