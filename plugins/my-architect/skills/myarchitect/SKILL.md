...
0. **Decide → Workflow I** — структурная работа (не «две корзины»-мелочь: дольше пары недель или трогает межсервисные контракты)? Сначала ценность и «делать ли»: гейт 7 вопросов → спина → апрув автора → эпик + ноды. Легитимные исходы: «не делаем» (Опция 0 / нет потребителя) и «не сейчас + триггер пересмотра». Мелкая работа идёт сразу в Workflow Z. Перед показом эпика бизнесу — **Workflow R** (`/my-architect:bo-review`, read-only, правки вносит автор).
