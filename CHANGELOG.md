...
### Added
- **«Убирай из запроса общие существительные»** — замер: `query "listPrompts createPrompt updatePrompt deletePrompt prompts owner_id"` вернул одноимённый файл `src/lg/prompts.ts` (про совсем другое) и родовой узел `Store`; тот же запрос БЕЗ слова `prompts` — ровно нужные методы с номерами строк. Одно частотное слово перетягивает выдачу на файл с тем же именем. Дополняет правило «спрашивай именами»: мало избегать прозы, надо ещё выкинуть родовые слова.
- **`graphify explain` часто лучше `query`** — когда символ известен, он отдаёт точный `файл:строка` плюс список ВЫЗЫВАЮЩИХ, чего `query` не даёт вовсе.
- **В монорепо каждый вызов из подкаталога падает.** `graphify` ищет `graphify-out/` относительно CWD, а индекс лежит в корне. Из `<repo>/<subproject>/` приходит `error: graph file not found`, и это легко принять за «графа нет». Лечится флагом `--graph <repo>/graphify-out/graph.json`.
- **Экспортируемые константы верхнего уровня в граф не попадают** (`export const X = {…}` / `= […]`). Экстрактор публикует функции и классы. Обидно конкретно потому, что реестры (`CONSUMPTION`, `FREE`, `API_PREFIXES`, `SCHEMA`, таблицы моделей) — ровно те места, куда чаще всего надо попасть.

## [1.18.1] — 2026-08-22

Багфикс-релиз: 1.18.0 научил сессию знать о графе, но три инструкции, которые он в неё впрыскивал, оказались неверными. Все правки ниже подтверждены прогонами на живом репозитории (9.5к узлов), а не рассуждением.

### Fixed
- **`graphify query "<question>"` → `graphify query "<SYMBOL AND FILE NAMES>"`** (`code-graph-context.sh`, `code-graph.md`). Прожитый отказ: агент честно пошёл в граф первым делом, задал прозаический вопрос — и получил маркетинговые документы вместо кода, после чего решил, что граф бесполезен, и разослал веер агентов. Замер A/B на одном вопросе: `"LibraryModal kindLock onPick uploadArtifact"` вернул нужные файлы и функции; «как пользователь выбирает картинку из библиотеки медиа» вернул чужой README и доки, кода — ноль. Причина структурная: doc-узлов в индексе почти столько же, сколько code-узлов (3870 против 5837), а метки у них — заголовки разделов, поэтому естественно-языковой запрос садится на заголовки и до символов не доходит. Инструкция сама вела в яму: она буквально предлагала спрашивать вопросом.
- **`graphify . --update` → `graphify update .`** (там же, 3 места). Форма из документа не обновляет индекс: голый путь трактуется CLI как полное извлечение и на репозитории с документами падает с `error: no LLM API key found (481 doc/paper/image file(s) need semantic extraction)`. Владелец, которому агент предложил эту команду по STALE-ветке, получал ошибку про ключ вместо обновления. `/graphify <path> --update` — слэш-команда скила, не CLI.
- **Свежесть — по `built_at_commit` внутри `graph.json`, а не по mtime файла.** mtime отвечает на вопрос «файл трогали», а не «с какого дерева собран индекс»: его двигает любая прерванная пересборка, копирование и сохранение в редакторе. Поле пишется каждой сборкой и сравнивается с `git rev-parse HEAD` точно. mtime оставлен фолбэком для индексов, собранных до появления поля, и в этом случае формулировка честно говорит, что судит слабее. Проверено мутацией: подменённый `built_at_commit` даёт STALE с обоими SHA.

### Added
- **`code-graph.md` → «Чего граф НЕ ВИДИТ вовсе: символов внутри тел функций».** Экстрактор берёт объявления верхнего уровня и внутрь тела не заходит. Замер: React-компонент на 4131 строку дал 17 узлов, последний из которых — сам компонент на L524, то есть 3600 строк тела дали ноль символов; верхнеуровневые функции того же файла извлеклись нормально. Плотность по расширениям: `.ts` 72.6 узла на 1000 строк, `.mjs` 40.7, `.tsx` 24.1. Опасное следствие, ради которого раздел и написан: **отсутствие символа в графе не доказывает, что его нет в коде** — это самая тихая форма ложного вывода из индекса.
- **`code-graph.md` → «Известные грабли эксплуатации»** — три отказа, каждый прожит:
  - *Визуализация исчезает молча и уносит старый файл.* Потолок HTML-виза — 5000 узлов; выше него хук печатает `Skipped graph.html` **и удаляет ранее собранный `graph.html`** (`watch.py`, `except ValueError` → `stale.unlink()`). Владелец видит только пропавшую картинку: пересборка идёт отсоединённым процессом, в терминал не пишет. Лечится строкой `export GRAPHIFY_VIZ_NODE_LIMIT` в git-хуках (её сотрёт повторный `graphify hook install` — так и написано). Альтернативы для больших графов: `graphify tree` и агрегат по сообществам. Заодно зафиксировано, что **`graphify export html` из SKILL.md в CLI 0.9.5 не существует** — есть только `export callflow-html`.
  - *Вендорные каталоги перевешивают свой код.* Механизма ignore нет; `.vite/deps` дал 661 узел, и чужой пре-бандл стал самым «густым» файлом проекта в графе. Лечится `rm -rf <vendor-dir> && graphify update . --force` (`--force` обязателен: при уменьшении числа узлов перезапись иначе отклоняется).
  - *Журнал пересборок общий на машину и не помечает репозиторий.* Всё пишется в `~/.cache/graphify-rebuild.log`; у владельца с двумя графифай-репо строки идут вперемешку, и подряд идущие `9110 → 405 → 9674` читаются как сбой сборки, хотя это два проекта. Перед диагностикой — сверить, сколько на машине `graphify-out/`. Скачки внутри одного репо объясняет `post-checkout`: смена ветки запускает полную пересборку, а не инкремент.

### Не сделано (осознанно, решение владельца)
- **Хук по-прежнему молчит, когда графа нет.** Предложение поставить graphify живёт только в `code-graph.md` → «Bootstrap по согласию», то есть срабатывает, лишь если агент сам решит вызвать скил — ровно та замкнутая петля, которую 1.18.0 разомкнул для случая «граф есть». Разомкнуть её и для «графа нет» — это впрыск контекста в каждую сессию каждого не-graphified проекта, а нулевая цена для таких проектов была явным решением 1.18.0. Менять её без владельца нельзя.
- Судьи по `trigger-evals.json` (21 кейс) с 1.18.0 так и не прогонялись.
